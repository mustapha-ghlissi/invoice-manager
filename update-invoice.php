...
{
  $invoice = $invoiceCRUD->showInvoice($_GET['id']);
  if($invoice !== null)
  {
    $invoice = $invoice[0];
    ...
    foreach ($invoiceRows as $key => $invoiceRow) {
        $tax = $taxCRUD->getTaxById($invoiceRow->getTax())[0];
...
if(isset($_POST['ref']))
{
  $exist = $invoiceCRUD->getInvoiceByRef($_POST['ref']);
  if($exist !== null && $exist[0]->getId() != $invoice->getId() )
  {
  ...
  $message = $invoiceCRUD->editInvoice($invoice, $_GET['id']);
  ...
  for($i = 0; $i < count($_POST['label']); $i++)
  {
    ...
    {
      if(in_array($i, array_keys($invoiceRows)))
      {
        $invoiceRow = $invoiceRows[$i];
        ...
        $invoiceRowCRUD->editInvoiceRow($invoiceRow);
      }
      else {
        ...
        $invoiceRow->setTax(intval(($_POST['tax'])[$i]));
        $invoiceRow->setNote(($_POST['note'])[$i]);
        $invoiceRow->setInvoice($invoice->getId());
  ...
  if(count($_POST['label']) < count($invoiceRows))
  {
    for($i = count($invoiceRows) - 1; $i>=count($_POST['label']); $i--)
    {
      $invoiceRowCRUD->deleteInvoiceRow($invoiceRows[$i]->getId());
    }
  }
  ...
  if(intval($_POST['status']) === 3)
  {
    ...
    $invoicePayment->setInvoice($invoice->getId());
    ...
    <div class="row" data-plugin="matchHeight" data-by-row="true">
      <div class="col-lg-12">
        <div class="panel">
          ...
          <div class="panel-body">
            <form name="invoiceForm" action="<?php echo($_SERVER['PHP_SELF']) ?>" id="editFormInvoice" method="post">
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="ref">
                    ...
                    <div class="input-group">
                      <input type="text" value="<?php echo $invoice->getRef() ?>" name="ref" id="ref" class="form-control form-control-sm" placeholder="Generate reference">
                ...
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <input type="text" name="startDate" value="<?php echo ($invoice->getStartDate());  ?>" data-plugin="datepicker" id="startDate" class="form-control form-control-sm" placeholder="Pick start date">
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <input type="text" name="endDate" value="<?php echo ($invoice->getEndDate());  ?>" data-plugin="datepicker" id="endDate" class="form-control form-control-sm" placeholder="Pick end date">
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control form-control-sm">
                      ...
                      <option value="1" <?php echo ($invoice->getStatus() === 1?"selected":"") ?>>
                        Draft
                      </option>
                      <option value="2" <?php echo ($invoice->getStatus() === 2?"selected":"") ?>>
                        Sent
                      </option>
                      <option value="3" <?php echo ($invoice->getStatus() === 3?"selected":"") ?>>
                        Paied
                      </option>
                    </select>
                  </div>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <textarea name="publicNote" id="publicNote" class="form-control form-control-sm" placeholder="Enter public note ..." rows="8" cols="80"><?php echo ($invoice->getPublicNote()) ?></textarea>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <textarea name="privateNote" id="privateNote" class="form-control form-control-sm" placeholder="Enter private note ..." rows="8" cols="80"><?php echo ($invoice->getPrivateNote()) ?></textarea>
                ...
                <div class="col-md-12">
                    ...
                    <tbody id="invoiceRows">
                      ...
                      foreach ($invoiceRows as $i => $invoiceRow) {
                        ...
                        <tr id="invoiceRow<?php echo ($i + 1); ?>">
                          ...
                          <td>
                            <input type="number" name="quantity[]" value="<?php echo ($invoiceRow->getQuantity()); ?>" min="1" data-id="<?php echo ($i + 1); ?>" id="quantity<?php echo ($i + 1); ?>" class="form-control form-control-sm">
                          </td>
                          <td>
                          ...
                          <td>
                            <input type="text" name="unityPrice[]" value="<?php echo (floatval($invoiceRow->getUnityPrice())); ?>" id="unityPrice<?php echo ($i + 1); ?>" data-id="<?php echo ($i + 1); ?>" class="form-control form-control-sm">
                          </td>
                          <td>
                            <input type="text" name="note[]" value="<?php echo ($invoiceRow->getNote()); ?>" class="form-control form-control-sm">
                          </td>
                          <td>
                            <?php
                              $taxPercentage = $invoiceRow->getTax();
                              $subtotal = $invoiceRow->getQuantity() * $invoiceRow->getUnityPrice();
                      ...
                      if(count($invoiceRows)<5)
                      {
                        for($i = count($invoiceRows) + 1; $i <= 5; $i++)
                        {
                          ?>
                      ...
                      <tfoot>
                        <tr class="btn-options">
                          <th colspan="5">
                            ...
                            <button class="btn btn-danger <?php if(count($invoiceRows)<=5) echo ('invisible');?>" id="btn-remove" type="button">
                          ...
                          <td>
                            ...
                            <input type="hidden" name="amount" id="amount" value="<?php echo ($total); ?>" class="form-control form-control-sm" placeholder="Enter the amount">
                          </td>
                        </tr>
                      </tfoot>
                    </table>
                  ...
                  <div class="col-md-12">
                    <div class="form-group text-right">
                      <button class="btn btn-primary" id="editInvoice" name="editInvoice">
                        Update invoice <i class="white-500 wb wb-file"></i>
                      </button>
                    </div>
                  </div>
                </div>
...
$(document).ready(function(){
  ...
  $('#editFormInvoice').on('submit',function(e){
    e.preventDefault();
    if($('#amount').val() === "")
    {
    ...
    editInvoice($(this), "<?php echo($_SERVER['PHP_SELF']) ?>?id=<?php echo ($_GET['id']); ?>");
  });
  ...
  $('#btn-remove').on('click', function(){
    ...
    if( size > 5 )
    {
      ...
      <?php
        ...
        {
      ?>
        var articleOption = $("<option></option>").val("<?php echo ($invoiceRow->getId()); ?>").text("<?php echo ($invoiceRow->getLabel()); ?>")
        $("#js-data-articles<?php echo ($i + 1); ?>").append(articleOption).trigger('change');
        $('#label<?php echo ($i + 1); ?>').val("<?php echo ($invoiceRow->getLabel()); ?>");
