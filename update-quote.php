...
if(isset($_GET['id']) && !empty($_GET['id']))
{
  ...
  if($quote !== null)
  {
    ...
    foreach ($quoteRows as $key => $quoteRow) {
        $tax = $taxCRUD->getTaxById($quoteRow->getTax())[0];
...
if(isset($_POST['ref']))
{
  $exist = $quoteCRUD->getQuoteByRef($_POST['ref']);
  if($exist !== null && $exist[0]->getId() != $quote->getId() )
  {
  ...
  $quote->setRepertory($_POST['repertory']);
  $quote->setRef($_POST['ref']);
  $creationDate = new \DateTime($_POST['creationDate']);
  $dueDate = new \DateTime($_POST['dueDate']);
  $quote->setCreationDate($creationDate);
  $quote->setDueDate($dueDate);
  $quote->setStatus($_POST['status']);
  $quote->setAmount(floatval($_POST['amount']));
  $quote->setPublicNote($_POST['publicNote']);
  $quote->setPrivateNote($_POST['privateNote']);
  $message = $quoteCRUD->editQuote($quote, $_GET['id']);
  ...
  for($i = 0; $i < count($_POST['label']); $i++)
  {
    ...
    {
      if(in_array($i, array_keys($quoteRows)))
      {
        $quoteRow = $quoteRows[$i];
        $quoteRow->setLabel(($_POST['label'])[$i]);
        $quoteRow->setQuantity(intval(($_POST['quantity'])[$i]));
        $quoteRow->setUnityPrice(floatval(($_POST['unityPrice'])[$i]));
        $quoteRow->setTax(intval(($_POST['tax'])[$i]));
        $quoteRow->setNote(($_POST['note'])[$i]);
        $quoteRowCRUD->editQuoteRow($quoteRow);
      }
      else {
        $quoteRow = new QuoteRow();
        $quoteRow->setLabel(($_POST['label'])[$i]);
        $quoteRow->setQuantity(intval(($_POST['quantity'])[$i]));
        $quoteRow->setUnityPrice(floatval(($_POST['unityPrice'])[$i]));
        $quoteRow->setTax(intval(($_POST['tax'])[$i]));
        $quoteRow->setNote(($_POST['note'])[$i]);
        $quoteRow->setQuote($quote->getId());
        $quoteRowCRUD->saveQuoteRow($quoteRow);
      }

    }
  }

  if(count($_POST['label']) < count($quoteRows))
  {
    for($i = count($quoteRows) - 1; $i>=count($_POST['label']); $i--)
    {
      $quoteRowCRUD->deleteQuoteRow($quoteRows[$i]->getId());
    }
  }
  ...
  if(intval($_POST['status']) === 3)
  {
    ...
    $invoice = new Invoice();
    $invoice->setQuote($quote->getId());
    $invoice->setRepertory($_POST['repertory']);
    $invoice->setRef($_POST['ref']);
    $startDate = new \DateTime($_POST['creationDate']);
    $endDate = new \DateTime($_POST['dueDate']);
    ...
    $invoiceCRUD = new InvoiceCRUD();
    $invoiceRowCRUD = new InvoiceRowCRUD();
    $invoice = $invoiceCRUD->getInvoiceByQuote($quote->getId());
    if($invoice !== null)
    {
      $invoice = $invoice[0];
      $invoiceCRUD->deleteInvoiceByQuote($quote->getId());
      $invoiceRowCRUD->deleteInvoiceRowByInvoice($invoice->getId());
    }
  }
    ...
    <div class="row" data-plugin="matchHeight" data-by-row="true">
      <div class="col-lg-12">
        <div class="panel">
          ...
          <div class="panel-body">
            <form name="quoteForm" action="<?php echo($_SERVER['PHP_SELF']) ?>" id="editFormQuote" method="post">
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="ref">
                    ...
                    <div class="input-group">
                      <input type="text" value="<?php echo $quote->getRef() ?>" name="ref" id="ref" class="form-control form-control-sm" placeholder="Generate reference">
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <select class="form-control form-control-sm js-data-repertories" name="repertory"></select>
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
                ...
                <input type="text" name="creationDate" value="<?php echo ($quote->getCreationDate());  ?>" data-plugin="datepicker" id="creationDate" class="form-control form-control-sm" placeholder="Pick start date">
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
                ...
                <input type="text" name="dueDate" value="<?php echo ($quote->getDueDate());  ?>" data-plugin="datepicker" id="dueDate" class="form-control form-control-sm" placeholder="Pick due date">
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
                ...
                <select name="status" id="status" class="form-control form-control-sm">
                  ...
                  <option value="1" <?php echo ($quote->getStatus() === 1?"selected":"") ?>>
                    Pending
                  </option>
                  <option value="2" <?php echo ($quote->getStatus() === 2?"selected":"") ?>>
                    Sent
                  </option>
                  <option value="3" <?php echo ($quote->getStatus() === 3?"selected":"") ?>>
                    Transformed
                  </option>
                </select>
              </div>
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <textarea name="publicNote" id="publicNote" class="form-control form-control-sm" placeholder="Enter public note ..." rows="8" cols="80"><?php echo ($quote->getPublicNote()) ?></textarea>
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <textarea name="privateNote" id="privateNote" class="form-control form-control-sm" placeholder="Enter private note ..." rows="8" cols="80"><?php echo ($quote->getPrivateNote()) ?></textarea>
            ...
            <div class="col-md-12">
              <table class="table table-responsive table-bordered">
                ...
                <tbody id="quoteRows">
                  <?php
                  foreach ($quoteRows as $i => $quoteRow) {
                    ?>
                    <tr id="quoteRow<?php echo ($i + 1); ?>">
                      <td><?php echo ($i + 1); ?></td>
                      <td>
                  ...
                  <td>
                    <input type="number" name="quantity[]" value="<?php echo ($quoteRow->getQuantity()); ?>" min="1" data-id="<?php echo ($i + 1); ?>" id="quantity<?php echo ($i + 1); ?>" class="form-control form-control-sm">
                  </td>
                  <td>
              ...
              <td>
                <input type="text" name="unityPrice[]" value="<?php echo (floatval($quoteRow->getUnityPrice())); ?>" id="unityPrice<?php echo ($i + 1); ?>" data-id="<?php echo ($i + 1); ?>" class="form-control form-control-sm">
              </td>
              <td>
                <input type="text" name="note[]" value="<?php echo ($quoteRow->getNote()); ?>" class="form-control form-control-sm">
              </td>
              <td>
                <?php
                $taxPercentage = $quoteRow->getTax();
                $subtotal = $quoteRow->getQuantity() * $quoteRow->getUnityPrice();
          ...
          if(count($quoteRows)<5)
          {
            for($i = count($quoteRows) + 1; $i <= 5; $i++)
            {
              ?>
              <tr id="quoteRow<?php echo ($i); ?>">
                <td><?php echo ($i); ?></td>
                <td>
          ...
          <tfoot>
            <tr class="btn-options">
              <th colspan="5">
                ...
                <button class="btn btn-danger <?php if(count($quoteRows)<=5) echo ('invisible');?>" id="btn-remove" type="button">
              ...
              <td>
                ...
                <input type="hidden" name="amount" id="amount" value="<?php echo ($quote->getAmount()); ?>" class="form-control form-control-sm" placeholder="Enter the amount">
              </td>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="col-md-12">
        <div class="form-group text-right">
          <button class="btn btn-primary" id="editQuote" name="editQuote">
            Edit quote <i class="white-500 wb wb-file"></i>
          </button>
        </div>
      </div>
    </div>
...
$(document).ready(function(){
  ...
  $('#editFormQuote').on('submit',function(e){
    e.preventDefault();
    if($('#amount').val() === "")
    {
    ...
    editQuote($(this), "<?php echo($_SERVER['PHP_SELF']) ?>?id=<?php echo ($_GET['id']) ?>");
  });

  $('#btn-add').on('click', function(){
    var size = ($('#quoteRows').children('tr').size());
  ...
  $('#btn-remove').on('click', function(){
    ...
    if( size > 5 )
    {
      $('#quoteRows').children('#quoteRow'+size).remove();
      if(size - 1 === 5)
      {
        if(!$('#btn-remove').hasClass('invisible'))
        {
  ...
  <?php
    ...
    {
  ?>
    var articleOption = $("<option></option>").val("<?php echo ($quoteRow->getId()); ?>").text("<?php echo ($quoteRow->getLabel()); ?>")
    $("#js-data-articles<?php echo ($i + 1); ?>").append(articleOption).trigger('change');
    $('#label<?php echo ($i + 1); ?>').val("<?php echo ($quoteRow->getLabel()); ?>");
